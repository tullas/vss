...
from vss_movie_creative_smoke import (
    MAXIMUM_ESTIMATED_COST_USD, MODEL_IDENTITY, PROVIDER_IDENTITY,
    SMOKE_3_EXPERIMENT_IDENTITY, AdmittedCreativeSmoke, SmokeProviderRequest,
)
...
def execute(context: CapabilityExecutionContext, input_data: dict[str, Any], dry_run: bool) -> CapabilityResult:
    ...
    if (context.environment != "development" or type(admitted) is not AdmittedCreativeSmoke
            or admitted.experiment_identity != SMOKE_3_EXPERIMENT_IDENTITY
            or input_data != {
                "admission_id": admitted.admission_id,
                "mode": "preflight" if dry_run else "generate",
            }):
        raise ValueError("creative smoke-3 admission boundary mismatch")
    common = {
        "experiment_status": "ready_for_paid_authorization" if dry_run else "generated_awaiting_human_review",
        ...
        "provider_identity": PROVIDER_IDENTITY, "model_identity": MODEL_IDENTITY,
        "maximum_provider_attempts": 1,
        "maximum_estimated_cost_usd": MAXIMUM_ESTIMATED_COST_USD,
        "readiness": {
            "credential_available": True, "direct_egress_environment": True, "dns_ready": True,
            "provider_call_count": 0 if dry_run else 1, "attempt_reserved": not dry_run,
        },
        "authority_boundary": AUTHORITY,
    }
    if dry_run:
        return CapabilityResult.success({
            **common, "artifact_path": None, "evidence_path": None, "review_path": None,
            "media_type": None, "width": None, "height": None, "byte_count": None,
            "content_sha256": None, "latency_ms": None, "provider_call_count": 0,
            "sanitized_usage": {}, "estimated_cost_usd": None, "png": None,
        })
    ...
    return CapabilityResult.success({
        **common, **paths, "media_type": result.media.media_type,
        "width": result.media.width, "height": result.media.height,
        "byte_count": len(result.media.content), "content_sha256": result.media.content_sha256,
        "latency_ms": result.latency_ms, "provider_call_count": result.provider_call_count,
        "sanitized_usage": dict(result.usage), "estimated_cost_usd": result.estimated_cost_usd,
        "png": result.png.as_dict(),
    })


execute.sdk_api_version = SDK_API_VERSION
execute.capability_identity = "movie.m8-3-real-provider-smoke-3"
execute.command_identity = "movie.m8-3-real-provider-smoke-3-generate"
