...
| ADRL-002 | Semantic evidence-binding pattern | `FRAMEWORK_CANDIDATE` | `WAIT_FOR_EVIDENCE` | M5.3, M6.3, and M6.4 repeat validation → exact semantic projection → invocation binding → independently validated result mechanics, while M6.2 supplies an assembly-boundary occurrence; M6.4 remains the same Movie domain, its lineage and recomputation invariants differ, and no safely removable cross-domain mechanism is yet isolated | Record increased evidence and Gateway-adapter pressure; keep validation and rules domain-owned; do not extract or promote the candidate | Measured maintenance evidence isolates stable removable mechanics across independent domains without erasing domain validation, or another independent domain demonstrates the same exact seam | Context / Semantic contracts | 2026-08-16 / M6.4 implementation evidence |
