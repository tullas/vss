...
| ADRL-002 | Semantic evidence-binding pattern | `FRAMEWORK_CANDIDATE` | `WAIT_FOR_EVIDENCE` | M5.3 and M6.3 now repeat the full validation → exact Context projection → invocation binding → independently validated result lifecycle, while M6.2 supplies a third assembly-boundary occurrence; the domain invariants and result recomputation remain materially different and no safely removable generic mechanism is yet isolated | Record increased evidence pressure; keep validation and rules domain-owned; do not extract or promote the candidate | Measured maintenance evidence isolates stable removable mechanics across these domains without erasing domain validation, or a further independent domain demonstrates the same exact seam | Context / Semantic contracts | 2026-08-16 / M6.3 implementation evidence |
