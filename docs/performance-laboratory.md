...
task, and family association. It never truncates developer audit state and
does not place raw lines in a report. The existing writer uses one append write
per bounded record for supported in-process concurrency. Local JSONL remains a
development facility; cross-process ordering, durability, integrity, rotation,
...
dependent; small changes are not fragile CI failures.
