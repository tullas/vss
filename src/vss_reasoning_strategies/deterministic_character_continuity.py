...
from vss_reasoning_providers.deterministic_character_continuity import DeterministicCharacterContinuityProvider, DeterministicCharacterContinuityAnalysisProvider
...
class DeterministicCharacterContinuityAnalysisStrategy:
    identity = "vss.analyze-character-continuity.deterministic"
    version = "1.1.0"
    provider_identity = "vss.reasoning.character-continuity.deterministic"
    ...
    def execute(self, view: CharacterContinuityProviderView, provider: DeterministicCharacterContinuityAnalysisProvider):
        if type(view) is not CharacterContinuityProviderView or type(provider) is not DeterministicCharacterContinuityAnalysisProvider:
            raise TypeError("character continuity analysis strategy substitution rejected")
        return provider.analyze(view), 1, 1
