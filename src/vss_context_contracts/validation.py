...
def validate_context(value: Any, registry: ContextContractRegistry) -> ValidatedContext:
    ...
    if value.get("context_family") == "character_continuity_context":
        from vss_movie_character_continuity import validate_character_continuity_context
        try:
            validated = validate_character_continuity_context(value, registry=registry)
            return ValidatedContext.create(validated.to_json_value())
        except Exception as exc:
