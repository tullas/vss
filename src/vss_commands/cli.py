...
def _parser() -> argparse.ArgumentParser:
    ...
    movie_experiment = movie_actions.add_parser("creative-reality-check-1")
    for flag in ("decision", "review-packet", "option-set", "scene-breakdown", "shot-plan", "storyboard"):
        movie_experiment.add_argument(f"--{flag}", type=Path, required=True)
    movie_experiment.add_argument("--frame-id", required=True)
    movie_experiment.add_argument("--environment", required=True)
    movie_experiment.add_argument("--correlation-id", required=True)
    movie_experiment.add_argument("--dry-run", action="store_true")
    movie_experiment.add_argument("--timeout", type=float)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.action == "movie":
        ...
        if args.movie_action == "prepare-option-review":
        ...
        elif args.movie_action in {"create-shot-plan-draft", "create-storyboard-specification", "render-storyboard", "generate-pictorial-frame", "creative-reality-check-1"}:
            ...
            shot_plan = None
            if args.movie_action in {"create-storyboard-specification", "render-storyboard", "generate-pictorial-frame", "creative-reality-check-1"}:
                shot_plan, shot_error = _read_context_file(args.shot_plan)
                input_error = input_error or shot_error
            storyboard = None
            if args.movie_action in {"render-storyboard", "generate-pictorial-frame", "creative-reality-check-1"}:
            ...
            if input_error is None:
                ...
                if args.movie_action not in {"render-storyboard", "generate-pictorial-frame", "creative-reality-check-1"}: input_data["request_id"] = args.request_id
                ...
                if args.movie_action == "creative-reality-check-1": input_data.update(frame_id=args.frame_id)
        ...
        if args.movie_action in {"context-assemble-character-continuity", "analyze-character-continuity", "prepare-option-review", "record-option-review-decision", "create-shot-plan-draft", "create-storyboard-specification", "render-storyboard", "generate-pictorial-frame", "creative-reality-check-1"}:
            pass
