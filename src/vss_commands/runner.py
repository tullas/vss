...
MOVIE_CREATIVE_EXPERIMENT_COMMAND = "movie.creative-reality-check-1"
        ...
        if command in {MOVIE_BREAKDOWN_COMMAND, MOVIE_CONTEXT_COMMAND, MOVIE_PROD_CONTEXT_COMMAND, MOVIE_PROD_COMMAND, MOVIE_REVIEW_COMMAND, MOVIE_REVIEW_DECISION_COMMAND, MOVIE_SHOT_PLAN_COMMAND, MOVIE_STORYBOARD_COMMAND, MOVIE_RENDER_COMMAND, MOVIE_PICTORIAL_COMMAND, MOVIE_CREATIVE_EXPERIMENT_COMMAND, MOVIE_CONTINUITY_CONTEXT_COMMAND, MOVIE_CONTINUITY_COMMAND}:
            try:
                ...
                if command == MOVIE_STORYBOARD_COMMAND:
                    ...
                    return finish("success", ExitCode.SUCCESS, result, [])
                if command == MOVIE_RENDER_COMMAND:
                    required = {"decision", "review_packet", "option_set", "scene_breakdown", "shot_plan", "storyboard"}
                    if frozenset(payload) != required:
                        return finish("error", ExitCode.INVALID_INPUT, {}, ["storyboard render input is invalid"])
                    ...
                    return runtime_controller.run(
                        command="movie.storyboard-render", environment=environment,
                        configuration=configuration, input_data={"admission_id": admitted.admission_id},
                ...
                if command == MOVIE_PICTORIAL_COMMAND:
                    required = {"decision", "review_packet", "option_set", "scene_breakdown", "shot_plan", "storyboard", "frame_id"}
                    if frozenset(payload) != required:
                        return finish("error", ExitCode.INVALID_INPUT, {}, ["pictorial frame input is invalid"])
                    from vss_movie_pictorial import admit_pictorial_frame
                    admitted = admit_pictorial_frame(
                        payload["decision"], payload["review_packet"], payload["option_set"],
                        payload["scene_breakdown"], payload["shot_plan"], payload["storyboard"],
                    ...
                    runtime_controller = self._runtime_controller or RuntimeController()
                    return runtime_controller.run(
                        command="movie.pictorial-frame-generate", environment=environment,
                        configuration=configuration, input_data={"admission_id": admitted.admission_id},
                        correlation_id=correlation, started_at=started_at, started_clock=started_clock,
                        dry_run=dry_run, timeout_seconds=timeout_seconds,
                        verbose=verbose, ask_become_pass=ask_become_pass, admitted_request=admitted,
                    )
                if command == MOVIE_CREATIVE_EXPERIMENT_COMMAND:
                    required = {"decision", "review_packet", "option_set", "scene_breakdown", "shot_plan", "storyboard", "frame_id"}
                    if frozenset(payload) != required:
                        return finish("error", ExitCode.INVALID_INPUT, {}, ["creative experiment input is invalid"])
                    from vss_movie_creative_experiment import admit_creative_experiment_plan
                    admitted = admit_creative_experiment_plan(
                        payload["decision"], payload["review_packet"], payload["option_set"],
                        payload["scene_breakdown"], payload["shot_plan"], payload["storyboard"],
                        frame_id=payload["frame_id"], environment=environment,
                    )
                    ...
                    runtime_controller = self._runtime_controller or RuntimeController()
                    return runtime_controller.run(
                        command="movie.creative-reality-check-1-generate", environment=environment,
                        configuration=configuration, input_data={"admission_id": admitted.by_condition["A"].admission_id},
                        correlation_id=correlation, started_at=started_at, started_clock=started_clock,
                        dry_run=dry_run, timeout_seconds=timeout_seconds,
                        verbose=verbose, ask_become_pass=ask_become_pass, admitted_request=admitted,
                    )
